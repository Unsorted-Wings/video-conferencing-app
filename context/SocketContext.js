...
const ContextProvider = ({ children }) => {
    ...
    const toggleVideo = () => {
        if (stream) {
            ...
            setVideoMuted(!stream.getVideoTracks()[0].enabled);
        }
    };

    return (
        <SocketContext.Provider value={{ call, callAccepted, myVideo, userVideo, stream, name, setName, callEnded, me, callUser, leaveCall, answerCall, toggleAudio, toggleVideo, audioMuted, videoMuted }}>
            {children}
        </SocketContext.Provider>
    );
};
